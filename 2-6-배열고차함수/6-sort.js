...
console.log(nums);  //정렬되지 않은 상태

nums.sort((a, b) => a - b); // 오름차
console.log(nums);  //[1, 2, 3, 4, 6, 7, 9, 10, 11, 27, 100]

nums.sort((a, b) => b - a); // 내림차
console.log(nums);  //[100, 27, 11, 10, 9, 7, 6, 4, 3, 2, 1]

// 문자 정렬
const foods = ['짜장면', '짬뽕', '가재튀김', '호빵', '닭강정'];

// 앞에데이터가 뒷데이터보다 크면 자리를 바꾸는데 양수일 때 자리가 바뀌도록 설계되어 있음.
foods.sort((a, b) => {
  if (a > b) return 1;  
